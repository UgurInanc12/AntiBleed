...
public final class DeviceManager: ObservableObject {
    ...
    /// Everything we create ourselves: the two driver devices and the private
    /// capture aggregate (`com.antibleed.aggregate.<UUID>`).
    public static let ownDeviceUIDPrefix = "com.antibleed."
    ...
    public func refresh() {
        ...
        for id in ids {
            ...
            if hidden { continue }
            // Never offer our own devices as a source: routing the virtual mic into
            // itself would feed back, and our private capture aggregate is an
            // implementation detail that must not become a persisted selection.
            if uid.hasPrefix(Self.ownDeviceUIDPrefix) { continue }
            if dev.isInput { inputs.append(dev) }
            if dev.isOutput { outputs.append(dev) }
        }
        // Presence is resolved by UID translation, NOT by scanning the list above.
        // The writer is deliberately hidden (D-005) and Apple's header is explicit
        // that hidden devices are not returned by kAudioHardwarePropertyDevices,
        // so looking for it there always failed and the app claimed the driver was
        // missing even when it was correctly installed (D-022).
        let micPresent = CoreAudioProperty.deviceID(forUID: Self.virtualMicUID) != nil
        let writerPresent = CoreAudioProperty.deviceID(forUID: Self.virtualWriterUID) != nil
